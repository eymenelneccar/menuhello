...
// إدارة التاريخ لضمان عمل زر الرجوع في الهاتف والمتصفح
...
window.showItemsForCategory = (category) => {
    ...
    try { refreshScrollAnimations && refreshScrollAnimations(); } catch {}
    // أضف حالة إلى التاريخ كي يعمل زر الرجوع في الهاتف والمتصفح
    try {
        if (!isHandlingPopstate) {
            history.pushState({ view: 'items', category }, '', `#${encodeURIComponent(category)}`);
        }
    } catch {}
}
...
// دالة عامة لزر الرجوع في الهيدر لتوحيد السلوك مع زر الهاتف
window.handleBack = () => {
    try { history.back(); } catch { showCategories(); }
};

// التعامل مع الرجوع/التقدم في التاريخ
window.addEventListener('popstate', (event) => {
    isHandlingPopstate = true;
    try {
        if (event.state && event.state.view === 'categories') {
            showCategories();
        } else if (event.state && event.state.view === 'items' && event.state.category) {
            // إذا تحرك المستخدم للأمام إلى حالة العناصر
            showItemsForCategory(event.state.category);
        } else {
            // حالة افتراضية
            showCategories();
        }
...
document.addEventListener('DOMContentLoaded', async () => {
  await loadAssets();
  // اضبط الحالة الابتدائية للفئات كي يعمل الرجوع إلى الصفحة السابقة عند التواجد على الفئات
  try { history.replaceState({ view: 'categories' }, '', window.location.href); } catch {}
  showCategories();
